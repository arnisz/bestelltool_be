...
INSERT INTO refresh_tokens(id, session_id, token_hash, family_id, successor_token_id, created_at, expires_at, revoked_at)
...
SELECT id, session_id, token_hash, family_id, successor_token_id, created_at, expires_at, revoked_at
FROM refresh_tokens
WHERE id = $1`, id)
...
SET successor_token_id = $2, revoked_at = $3
WHERE id = $1`,
		token.ID,
		token.SuccessorTokenID,
		token.RevokedAt,
