...
## 🎯 Current Focus
**Phase 1, Punkt 2+3 (Code fertig, Verifikation ausstehend):** Migration `000006` (`audit_events.actor_role` um `admin`, `entity_type` um administrative Werte erweitert; Append-Only-Trigger um `TRUNCATE` und SQLSTATE `42501` gehärtet) sowie das zugehörige Test-Cleanup-Refactoring (Schema-Reset statt `TRUNCATE`) sind implementiert. Noch offen: Lauf gegen die echte Testdatenbank (zwei Läufe hintereinander gegen denselben Container, siehe Abschlussbericht) — laut `agents.md` sind grüne Skips ohne echten DB-Lauf nicht ausreichend, sobald die Umgebung verfügbar ist. Punkte 2/3 werden erst nach diesem Nachweis als abgeschlossen markiert.
...
- **Audit-Schema-Erweiterung (Migration `000006`) implementiert, aber gegen die echte Testdatenbank noch nicht verifiziert**: `actor_role` kennt jetzt `admin`, `entity_type` kennt die administrativen Typen. Solange der reale DB-Lauf (siehe Abschlussbericht, zwei Läufe gegen denselben Container) nicht bestätigt wurde, gilt Phase 1 (Punkt 2/3) als nicht final abgeschlossen. Admin-Endpunkte dürfen weiterhin **nicht** ausgeliefert werden, bis diese Verifikation vorliegt.
- **Audit-Unveränderlichkeit**: seit Migration `000002` per Trigger technisch erzwungen (nicht nur Design-Zusage); seit `000006` zusätzlich mit stabilem SQLSTATE `42501` und `TRUNCATE`-Schutz. Die in `000006` ergänzten `REVOKE`-Statements sind **vorbereitend, nicht wirksam**: Backend, Migrationen und .NET-Tool laufen weiterhin unter demselben (Eigentümer-)Konto, das von `REVOKE ... FROM PUBLIC` nicht betroffen ist. Echte Durchsetzung erst mit der Rollentrennung in Phase 5 (siehe `docs/deployment.md` Abschnitt 6).
