...
## 🎯 Current Focus
**Phase 2 (Auth-Kern), Punkte 5–7 sind umgesetzt und verifiziert** (Migrationen `000007`/`000008`, `domain.User` erweitert, `UserRepository` angepasst, neue Ports `AuthIdentityRepository`/`SessionRepository`/`RefreshTokenRepository`/`PasswordHasher`/`SecretGenerator`/`Clock` — siehe Completed-Eintrag unten). Nächster Schritt: Punkt 8 — Argon2id-Adapter (`internal/adapters/auth/argon2`) mit PHC-Kodierung, konfigurierbaren Parametern, `NeedsRehash` und Rehash-on-Login (SEC-02).
...
- **`-race` in dieser Entwicklungsumgebung nicht ausführbar**: kein C-Compiler installiert (cgo-Voraussetzung für den Go-Race-Detector). Die Concurrency-relevanten Tests (Last-Admin-Guard bei SEC-18, künftig) müssen spätestens vor deren Implementierung auf einer Maschine mit funktionierendem C-Toolchain gegen `db-test` mit `-race` verifiziert werden — siehe `agents.md` §11.3.
