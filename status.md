...
## 🎯 Current Focus
**Phase 1 (Audit-Fundament) ist vollständig abgeschlossen und gegen die echte Testdatenbank verifiziert (Punkte 2, 3, 4).** Nächster Schritt: Tech Debt vor Phase 2, Punkt 5 — `seedDirectTransferPreconditions` (E2E-Test) und `scripts/dev-seed.sql` von handgeschriebenem SQL auf Seeding über Repositories/Use Cases innerhalb einer UnitOfWork umstellen, bevor Migration `000007` (`users.username` als `NOT NULL` + `UNIQUE`) den bestehenden Drift zum Bruch macht.
